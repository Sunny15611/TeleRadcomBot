...
export function ChatBox({ chatId }: { chatId?: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const response = await fetch("https://dc82-103-147-138-252.ngrok-free.app/api/query1", {
          method: "POST",
          headers: {
        ...
        while (true) {
          ...
          for (const line of lines) {
            if (line.startsWith("data: ")) {
              ...
              if (typeof data.token === "string" && data.token && data.token.length < 20) {
                ...
                console.log(`the assistnat response is ${assistantResponse}`)
                console.log(`documents are ${data.documents}`);
                console.log(`Images are  ${data.imageData}`);
              const temp = data;
                setMessages((prev) => {
                ...
                await sendMessage({
                  ...
                  imageData: JSON.stringify(temp.imageData[0]) as string,
                })
                  .then((response) => {
